...
export function loadWasm<T extends FnBag>(instance: T): WasmLoaderReturn<T> {
  ...
  const methodNames = Object.keys(instance).filter(
    (i) => i !== "default"
  ) as Array<keyof T>;
    ...
    const asyncHandler = async (...args: any[]) => {
      ...
      try {
        ...
        if (err.message === "unreachable") {
          const { stack, _message } = err;
          console.log(err);
          err = new Error(
            `An unknown error was thrown while executing the '${name.toString()}()' handler in your wasm module.`
          );
          err.stack = stack;
        }
